...
<div id="formWrap">
    <form action="hLogWrite.jsp" method="post">
        <fieldset>
            <legend class="screen_out">H_Log_Wrap</legend>
            <div id="heroWrap">
                <div id="colorWrap">
                    <input type="hidden" id="heroImgName" name="titlePic"/>
                ...
                <div class="travelGrade">
                    <div class="travelBox">
                        ...
                        <ul class="searchTag travel">
                            <li><label>#<input class="inputTag" placeholder="태그1" name="tag1" autocomplete="off"/></label><ul class="tagList"><li class="tags">테스트</li></ul></li>
            ...
            <button class="hLogBtn" type="button">등록</button>
            </div><!--//contentsWrap-->
        </fieldset>
    </form>
</div><!--//formWrap-->
...
<script>
    ...
    editor.eventManager.addEventType('textEventLeft');
    editor.eventManager.addEventType('textEventCenter');

    editor.eventManager.listen('imageEvent', function() {
        $uploadImage.click();
    });//listen() end

    editor.eventManager.listen('textEventLeft', function() {
        $(".textAlignLeft").click();
    });//listen() end

    editor.eventManager.listen('textEventCenter', function() {
    ...
    toolbar.addButton({
        ...
        event: 'textEventLeft',
        tooltip: '왼쪽정렬',
        $el: $('<button class="btn_image"><i class="fas fa-align-left"></i></button>')
    });

    toolbar.addButton({
        name: 'textAlignCenter',
        event: 'textEventCenter',
        tooltip: '가운데정렬',
        $el: $('<button class="btn_image"><i class="fas fa-align-center"></i></button>')
    });
    ...
    $(document).on("click",".textAlignCenter",function() {
        $('.tui-editor-contents').removeClass("textLeft");
        $('.tui-editor-contents').addClass("textCenter");

    });

    $(document).on("click",".textAlignLeft",function() {
        $('.tui-editor-contents').removeClass("textCenter");
        $('.tui-editor-contents').addClass("textLeft");
